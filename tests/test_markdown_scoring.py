...
never assert on `distance`/`direction`, only on the new `markdown_report` / `markdown`
...
class StructScoringTests(unittest.TestCase):
    def test_axes_cover_every_named_struct_axis(self):
        axes = _model().markdown_report(_EXEMPLARS[0])
    ...
    def test_zero_variance_axis_is_finite_and_ontarget(self):
        ...
        model = _model()
        axes = {a.axis: a for a in model.markdown_report(_EXEMPLARS[0])}
        ...
        for a in model.markdown_report(_EXEMPLARS[0]):
    ...
    def test_draft_with_no_markdown_structure(self):
        ...
        model = _model()
        axes = {a.axis: a for a in model.markdown_report("just a sentence of plain prose.")}
