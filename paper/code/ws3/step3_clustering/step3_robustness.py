...
Run (from repo root):
  ...
  uv run --with-requirements paper/code/ws3/requirements.txt \
      python paper/code/ws3/step3_clustering/step3_robustness.py
"""
...
def _write_summary(path: Path, root, G, R, **kw) -> None:
    ...
    L.append(
        "Generated by `paper/code/ws3/step3_clustering/step3_robustness.py`. Numbers here are the ones cited "
...
def _append_surgical_summary(path: Path, root, G, R, **kw) -> None:
    ...
    L.append(
        "Appended by `paper/code/ws3/step3_clustering/step3_robustness.py --tight-only` (LEDGER PRE-REG "
