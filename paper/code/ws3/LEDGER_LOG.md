...
Full PRE-REG + RESULT blocks for every WS3 experiment, verbatim, each under a stable
`## <ID> · <title>` heading. Newest experiment on top. Join key with the INDEX
(`../ws3/LEDGER.md`) is the ID. See the INDEX's "Format & conventions" header for
shared cross-experiment rules (metric identity, seed, timestamp format, gotchas).
...
- **`featurize.py` step-1 scaffold (2026-07-08)** — superseded-by-WS3-1-EXTRACT.
  Scaffolded + unit-tested (TDD, ponytail): pure seam `featurize_rows` filters
  `is_canonical`, carries metadata forward. Superseded for the real run by
  `extract_features.py` (2026-07-09, see WS3-1-EXTRACT): the scaffold was
  canonical-only, in-memory (`to_pylist()`), non-parallel, non-resumable, and deferred
  the manifest — `extract_features.py` adds all of those plus the ADR-0010
  install-labeled union scope needed for RQ2. Scaffold + its `test_featurize.py` kept
  as-is in the repo (pure-seam reference), not deleted.
